...
import dev.yanianz.sourbyanticheat.utils.data.TrackerData;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.DashableEntity;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntity;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntityHook;
...
public class PacketEntityReplication extends Check implements PacketCheck {
    ...
    public PacketEntityReplication(SacPlayer player) {
        super(player);
    }
            ...
            if (slot.getWindowId() == 0) {
                Runnable task = () -> {
                    if (slot.getSlot() - 36 == player.packetStateData.lastSlotSelected && (
                        ...
                        if (player.isResetItemUsageOnItemUpdate() && hand == SacAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer)) {
                            SacAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                        }
                    }
                };
                ...
                Runnable task = () -> {
                    if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_8)) {
                        ...
                        if (items.getItems().size() > 45 && !player.inventory.getOffHand().is(items.getItems().get(45).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && SacAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.OFF_HAND) {
                                SacAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                        ...
                        if (!player.inventory.getHeldItem().is(items.getItems().get(player.packetStateData.lastSlotSelected + 36).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && SacAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.MAIN_HAND) {
                                SacAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                    }
    ...
    private boolean isDirectlyAffectingPlayer(SacPlayer player, int entityID) {
        // The attributes for this entity is active, currently
