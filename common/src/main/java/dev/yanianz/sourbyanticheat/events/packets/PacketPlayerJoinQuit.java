...
public class PacketPlayerJoinQuit extends PacketListenerAbstract {
    ...
    public void onPacketSend(PacketSendEvent event) {
        if (event.getPacketType() == PacketType.Login.Server.LOGIN_SUCCESS) {
            ...
            event.getTasksAfterSend().add(() -> {
                SacAPI.INSTANCE.getPlayerDataManager().addUser(event.getUser());
                ...
                if (uuid != null) {
                    SacAPI.INSTANCE.getDataStoreLifecycle().playerToggleStore().prefetch(uuid);
                }
            });
        }
    }
    ...
    public void onUserConnect(UserConnectEvent event) {
        ...
        if (event.getUser().getConnectionState() == ConnectionState.PLAY && !SacAPI.INSTANCE.getPlayerDataManager().exemptUsers.contains(event.getUser())) {
    ...
    public void onUserLogin(UserLoginEvent event) {
        ...
        @NotNull PlatformPlayer platformPlayer = SacAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(nativePlayerObject);

        if (SacAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("debug-pipeline-on-join", false)) {
        ...
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_ALERTS,
                "sac.alerts", "sac.alerts.enable-on-join", "sac.alerts.enable-on-join.silent",
                (p, silent) -> SacAPI.INSTANCE.getAlertManager().toggleAlerts(p, silent),
                (p, value) -> SacAPI.INSTANCE.getAlertManager().setAlertsEnabled(p, value, true));
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_VERBOSE,
                "sac.verbose", "sac.verbose.enable-on-join", "sac.verbose.enable-on-join.silent",
                (p, silent) -> SacAPI.INSTANCE.getAlertManager().toggleVerbose(p, silent),
                (p, value) -> SacAPI.INSTANCE.getAlertManager().setVerboseEnabled(p, value, true));
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_BRANDS,
                "sac.brand", "sac.brand.enable-on-join", "sac.brand.enable-on-join.silent",
                (p, silent) -> SacAPI.INSTANCE.getAlertManager().toggleBrands(p, silent),
                (p, value) -> SacAPI.INSTANCE.getAlertManager().setBrandsEnabled(p, value, true));

        if (platformPlayer.hasPermission("sac.spectate") && SacAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("spectators.hide-regardless", false)) {
            SacAPI.INSTANCE.getSpectateManager().onLogin(platformPlayer.getUniqueId());
        }
    ...
    public void onUserDisconnect(UserDisconnectEvent event) {
        ...
        SacAPI.INSTANCE.getPlayerDataManager().onDisconnect(event.getUser());
    }
}
