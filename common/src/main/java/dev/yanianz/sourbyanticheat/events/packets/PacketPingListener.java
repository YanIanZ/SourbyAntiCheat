...
public class PacketPingListener extends PacketListenerAbstract {
    ...
    private static final GrimTransactionSendEvent.Channel SEND_CHANNEL = SacAPI.INSTANCE.getEventBus().get(GrimTransactionSendEvent.class);
    private static final GrimTransactionReceivedEvent.Channel RECEIVED_CHANNEL = SacAPI.INSTANCE.getEventBus().get(GrimTransactionReceivedEvent.class);
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        if (event.getPacketType() == PacketType.Play.Client.WINDOW_CONFIRMATION) {
            ...
            if (id <= 0 && player.addTransactionResponse(id)) {
                ...
                boolean shouldCancel = !SacAPI.INSTANCE.getConfigManager().isDisablePongCancelling();
