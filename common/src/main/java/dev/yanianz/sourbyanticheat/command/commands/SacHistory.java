...
import dev.yanianz.sourbyanticheat.command.render.HistoryComponentRenderer;
...
import dev.yanianz.sourbyanticheat.platform.api.player.OfflinePlatformPlayer;
 ...
 */
public class SacHistory implements BuildableCommand {
    ...
    private void runWithPrelude(Sender sender, String target, HistoryAction action) {
        DataStoreLifecycle lifecycle = SacAPI.INSTANCE.getDataStoreLifecycle();
        ...
        try {
            OfflinePlatformPlayer targetPlayer =
                    SacAPI.INSTANCE.getPlatformPlayerFactory().getOfflineFromName(target);
    ...
    private static SuggestionProvider<Sender> targetSuggestions(CloudCommandAdapter adapter) {
        ...
        return SuggestionProvider.blocking((ctx, in) -> {
            ...
            DataStoreLifecycle dsl = SacAPI.INSTANCE.getDataStoreLifecycle();
    ...
    private static @Nullable UUID resolveTargetUuid(CommandContext<Sender> ctx) {
        ...
        OfflinePlatformPlayer p = SacAPI.INSTANCE.getPlatformPlayerFactory().getOfflineFromName(target);
    ...
    private Component message(String key, String fallback, Map<String, String> vars) {
        String raw = SacAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallback);
