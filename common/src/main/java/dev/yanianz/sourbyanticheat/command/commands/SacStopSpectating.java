...
import dev.yanianz.sourbyanticheat.command.requirements.PlayerSenderRequirement;
...
public class SacStopSpectating implements BuildableCommand {
    ...
    public void onStopSpectate(CommandContext<Sender> commandContext) {
        ...
        if (SacAPI.INSTANCE.getSpectateManager().isSpectating(sender.getUniqueId())) {
            boolean teleportBack = string == null || !string.equalsIgnoreCase("here") || !sender.hasPermission("sac.spectate.stophere");
            SacAPI.INSTANCE.getSpectateManager().disable(Objects.requireNonNull(sender.getPlatformPlayer()), teleportBack);
        } else {
