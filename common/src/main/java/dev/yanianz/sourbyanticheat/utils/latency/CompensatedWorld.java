package dev.yanianz.sourbyanticheat.utils.latency;
...
import dev.yanianz.sourbyanticheat.utils.data.BlockPrediction;
import dev.yanianz.sourbyanticheat.utils.data.Pair;
import dev.yanianz.sourbyanticheat.utils.data.PistonData;
import dev.yanianz.sourbyanticheat.utils.data.ShulkerData;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntity;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntityShulker;
...
public class CompensatedWorld implements PacketWorld {
    ...
    public CompensatedWorld(SacPlayer player) {
        this.player = player;
    ...
    public void stopPredicting(PacketWrapper<?> wrapper) {
        ...
        if (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_19)) {
            ...
            SacAPI.INSTANCE.getScheduler().getGlobalRegionScheduler().run(SacAPI.INSTANCE.getGrimPlugin(), () -> {
    ...
    public double getWaterFluidLevelAt(double x, double y, double z) {
        return getWaterFluidLevelAt(SacMath.floor(x), SacMath.floor(y), SacMath.floor(z));
    }
