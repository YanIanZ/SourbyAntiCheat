...
public class ReachUtils {
    ...
    @Contract("_, _, _ -> new")
    public static @NotNull Vector3dm getLook(@NotNull SacPlayer player, float yaw, float pitch) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_12_2)) {
            float yawRadians = SacMath.radians(-yaw) - (float) Math.PI;
            float pitchRadians = SacMath.radians(-pitch);
            ...
            float pitchRadians = SacMath.radians(pitch);
            float yawRadians = SacMath.radians(-yaw);
            float pitchCos = player.trigHandler.cos(pitchRadians);
            float x = player.trigHandler.sin(yawRadians);
            float y = player.trigHandler.sin(pitchRadians);
            float z = player.trigHandler.cos(yawRadians);
    ...
    public static double getMinReachToBox(@NotNull SacPlayer player, @NotNull SimpleCollisionBox targetBox) {
