...
public class JumpPower {
    public static void jumpFromGround(@NotNull SacPlayer player, @NotNull Vector3dm vector) {
        ...
        if (player.isSprinting) {
            float radRotation = SacMath.radians(player.yaw);
    ...
    public static float getJumpPower(@NotNull SacPlayer player) {
        return (float) player.compensatedEntities.self.getAttributeValue(Attributes.JUMP_STRENGTH) * getPlayerJumpFactor(player);
    }

    public static float getPlayerJumpFactor(@NotNull SacPlayer player) {
