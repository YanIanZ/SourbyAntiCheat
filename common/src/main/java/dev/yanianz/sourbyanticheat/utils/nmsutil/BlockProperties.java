...
import dev.yanianz.sourbyanticheat.utils.data.MainSupportingBlockData;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntity;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntityHorse;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntityNautilus;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntityStrider;
...
public class BlockProperties {
    public static float getFrictionInfluencedSpeed(float f, SacPlayer player) {
     ...
     */
    public static StateType getOnPos(SacPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static float getFriction(SacPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)) {
    ...
    public static float getBlockSpeedFactor(SacPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static boolean onHoneyBlock(SacPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
     ...
     */
    private static StateType getBlockPosBelowThatAffectsMyMovement(SacPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        Vector3i pos = player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)
                ? new Vector3i(SacMath.floor(playerPos.getX()), SacMath.floor(playerPos.getY() - 0.5000001), SacMath.floor(playerPos.getZ()))
                : getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getBlockType(pos.x, pos.y, pos.z);
    }

    private static Vector3i getOnPos(SacPlayer player, Vector3d playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.withY(SacMath.floor(playerPos.getY() - (double) searchBelowPlayer)) : mainBlockPos;
        } else {
            return new Vector3i(SacMath.floor(playerPos.getX()), SacMath.floor(playerPos.getY() - searchBelowPlayer), SacMath.floor(playerPos.getZ()));
        }
    }

    public static float getMaterialFriction(SacPlayer player, StateType material) {
        float friction = 0.6f;
    ...
    private static StateType getOnBlock(SacPlayer player, double x, double y, double z) {
        StateType block1 = player.compensatedWorld.getBlockType(SacMath.floor(x), SacMath.floor(y - 0.2F), SacMath.floor(z));

        if (block1.isAir()) {
            StateType block2 = player.compensatedWorld.getBlockType(SacMath.floor(x), SacMath.floor(y - 1.2F), SacMath.floor(z));
    ...
    private static float getBlockSpeedFactorLegacy(SacPlayer player, Vector3d pos) {
    ...
    private static float getBlockSpeedFactor(SacPlayer player, StateType type) {
    ...
    private static float getModernVelocityMultiplier(SacPlayer player, float blockSpeedFactor) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) return blockSpeedFactor;
        return (float) SacMath.lerp((float) player.compensatedEntities.self.getAttributeValue(Attributes.MOVEMENT_EFFICIENCY), blockSpeedFactor, 1.0F);
    }
}
