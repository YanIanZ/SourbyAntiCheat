package dev.yanianz.sourbyanticheat.utils.data.attribute;
...
import static dev.yanianz.sourbyanticheat.utils.latency.CompensatedEntities.SPRINTING_MODIFIER_UUID;
...
public final class ValuedAttribute {
     ...
     */
    public ValuedAttribute requiredVersion(SacPlayer player, ClientVersion requiredVersion) {
    ...
    public double with(WrapperPlayServerUpdateAttributes.Property property) {
        ...
        double newValue =
                SacMath.clamp((baseValue + additionSum) * (1 + multiplyBaseSum) * multiplyTotalProduct,
                        min, max);
        if (setRewriter != null) {
