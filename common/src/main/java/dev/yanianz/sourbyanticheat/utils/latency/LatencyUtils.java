...
public class LatencyUtils {
    ...
    public LatencyUtils(SacPlayer player) {
        this.player = player;
    }
            ...
            for (Runnable runnable : tasksToRun) {
                try {
                    ...
                    if (CommonGrimArguments.KICK_ON_TRANSACTION_ERRORS.value()) {
                        player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, SacAPI.INSTANCE.getConfigManager().getDisconnectPacketError())));
                    }
                }
            }
        }
