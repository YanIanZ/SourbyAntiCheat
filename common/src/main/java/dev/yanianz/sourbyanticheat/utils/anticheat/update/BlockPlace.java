package dev.yanianz.sourbyanticheat.utils.anticheat.update;

import dev.yanianz.sourbyanticheat.player.SacPlayer;
import dev.yanianz.sourbyanticheat.utils.anticheat.LogUtil;
import dev.yanianz.sourbyanticheat.utils.collisions.AxisSelect;
...
public class BlockPlace {
    ...
    public BlockPlace(SacPlayer player, InteractionHand hand, Vector3i position, int faceId, BlockFace face, ItemStack itemStack, @Nullable HitData hitData, int sequence) {
        this.player = player;
        this.hand = hand;
    ...
    private List<BlockFace> getNearestLookingDirections() {
        float pitch = SacMath.radians(player.pitch);
        float yaw = SacMath.radians(-player.yaw);
    ...
    public BlockFace getPlayerFacing() {
        return BY_2D[SacMath.floor(player.yaw / 90.0D + 0.5D) & 3];
    }
