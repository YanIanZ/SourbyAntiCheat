...
import dev.yanianz.sourbyanticheat.utils.data.webhook.discord.CompiledDiscordTemplate;
...
public class MessageUtil {
    ...
    public @Nullable String replacePlaceholders(@Nullable SacPlayer player, @Nullable String string, boolean removeFormatting) {
        return replacePlaceholders(player, player == null ? null : player.platformPlayer, string, removeFormatting);
    }

    @Contract("_, null -> null; _, !null -> !null")
    public @Nullable String replacePlaceholders(@Nullable SacPlayer player, @Nullable String string) {
    ...
    public @Nullable String replacePlaceholders(@Nullable PlatformPlayer player, @Nullable String string) {
        return replacePlaceholders(player == null ? null : SacAPI.INSTANCE.getPlayerDataManager().getPlayer(player.getUniqueId()), player, string, false);
    }
    ...
    private @Nullable String replacePlaceholders(@Nullable SacPlayer grimPlayer, @Nullable PlatformPlayer platformPlayer, @Nullable String string, boolean removeFormatting) {
        ...
        if (!matcher.find()) {
            return SacAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, string);
        }

        // Get references to the maps once, outside the loop.
        final Map<String, String> staticReplacements = SacAPI.INSTANCE.getExternalAPI().getStaticReplacements();
        final Map<String, Function<GrimUser, String>> variableReplacements = SacAPI.INSTANCE.getExternalAPI().getVariableReplacements();
        ...
        return SacAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, grimReplaced).replace(PLACEHOLDER_ESCAPE_CHAR, '%');
    }

    public @NotNull Component replacePlaceholders(@NotNull SacPlayer player, @NotNull Component component) {
    ...
    public @NotNull Component miniMessage(@NotNull String string) {
        string = string.replace("%prefix%", SacAPI.INSTANCE.getConfigManager().getConfig().getStringElse("prefix", "&bGrim &8»"));
    ...
    public Component getParsedComponent(Sender sender, String key, String fallbackText) {
        String message = SacAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallbackText);
