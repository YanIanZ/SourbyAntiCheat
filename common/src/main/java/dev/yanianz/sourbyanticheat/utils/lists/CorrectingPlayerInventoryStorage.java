package dev.yanianz.sourbyanticheat.utils.lists;
...
public class CorrectingPlayerInventoryStorage extends InventoryStorage {
    ...
    public CorrectingPlayerInventoryStorage(SacPlayer player, int size) {
        super(size);
        this.player = player;
    }
    ...
    public void handleClientClaimedSlotSet(int slotID) {
        if (slotID >= 0 && slotID <= Inventory.ITEMS_END) {
            pendingFinalizedSlot.put(slotID, SacAPI.INSTANCE.getTickManager().currentTick + 5);
        }
    }
    ...
    public void setItem(int item, ItemStack stack) {
        ...
        if (finalTransaction == -1 || player.lastTransactionReceived.get() >= finalTransaction) {
            ...
            pendingFinalizedSlot.put(item, SacAPI.INSTANCE.getTickManager().currentTick + 5);
    ...
    public void tickWithBukkit() {
        ...
        int tickID = SacAPI.INSTANCE.getTickManager().currentTick;
