package dev.yanianz.sourbyanticheat.utils.nmsutil;
...
import dev.yanianz.sourbyanticheat.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_10;
import dev.yanianz.sourbyanticheat.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_2;
import dev.yanianz.sourbyanticheat.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_4;
import dev.yanianz.sourbyanticheat.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_5;
import dev.yanianz.sourbyanticheat.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_6;
import dev.yanianz.sourbyanticheat.utils.chunks.Column;
import dev.yanianz.sourbyanticheat.utils.collisions.CollisionData;
import dev.yanianz.sourbyanticheat.utils.collisions.datatypes.CollisionBox;
...
import dev.yanianz.sourbyanticheat.utils.data.tags.SyncedTags;
...
public final class Collisions {
    ...
    public static boolean slowCouldPointThreeHitGround(SacPlayer player, double x, double y, double z) {
    ...
    public static Vector3dm collide(SacPlayer player, double desiredX, double desiredY, double desiredZ) {
        return collide(player, desiredX, desiredY, desiredZ, desiredY, null);
    }

    public static Vector3dm collide(SacPlayer player, double desiredX, double desiredY, double desiredZ, double clientVelY, VectorData data) {
    ...
    public static boolean addWorldBorder(SacPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean getCollisionBoxes(SacPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean isEmpty(SacPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static Vector3dm maybeBackOffFromEdge(Vector3dm vec3, SacPlayer player, boolean overrideVersion) {
    ...
    public static boolean isAboveGround(SacPlayer player) {
    ...
    public static void handleInsideBlocks(SacPlayer player) {
    ...
    public static void onInsideBlock(SacPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, StateType blockType, WrappedBlockState block, int blockX, int blockY, int blockZ, boolean magic) {
    ...
    public static void applyEffectsFromBlocks(SacPlayer player) {
        ...
        if (clientVersion.isOlderThan(ClientVersion.V_1_21_5)) {
            player.finalMovementsThisTick.add(new SacPlayer.Movement(from, to));
            ...
            if (player.finalMovementsThisTick.isEmpty()) {
                ...
                player.finalMovementsThisTick.add(new SacPlayer.Movement(player.finalMovementsThisTick.get(player.finalMovementsThisTick.size() - 1).to(), to));
            }
        }
    ...
    public static void resolveBlockEffects(SacPlayer player, Vector3d from, Vector3d to) {
        Collisions.resolveBlockEffects(player, List.of(new SacPlayer.Movement(from, to)));
    }

    public static void resolveBlockEffects(SacPlayer player, List<SacPlayer.Movement> movements) {
        resolveBlockEffects(player, player.clientVelocity, false, movements);
    }

    public static void resolveBlockEffects(SacPlayer player, Vector3dm clientVelocity, boolean onlyApplyVelocity, List<SacPlayer.Movement> movements) {
    ...
    private static double getOldDeltaY(SacPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? value / 0.98F + 0.08 : value;
    }

    private static double getNewDeltaY(SacPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? (value - 0.08) * 0.98F : value;
    }

    private static boolean isSlidingDown(Vector3dm vector, SacPlayer player, int locationX, int locationY,
                                         int locationZ) {
        if (player.onGround) {
            return false;
    ...
    // 0.03 hack
    public static boolean checkStuckSpeed(SacPlayer player, double expand) {
        // Use the bounding box for after the player's movement is applied
    ...
    public static boolean suffocatesAt(SacPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static boolean doesBlockSuffocate(SacPlayer player, int x, int y, int z) {
        WrappedBlockState data = player.compensatedWorld.getBlock(x, y, z);
    ...
    // Thanks Tuinity
    public static boolean hasMaterial(SacPlayer player, SimpleCollisionBox checkBox, Predicate<Pair<WrappedBlockState, Vector3i>> searchingFor) {
    ...
    // Thanks Tuinity
    public static void forEachCollisionBox(@NotNull SacPlayer player, @NotNull SimpleCollisionBox checkBox, @NotNull BiConsumer<WrappedBlockState, Vector3i> searchingFor) {
    ...
    public static boolean onClimbable(SacPlayer player, double x, double y, double z) {
    ...
    public static boolean trapdoorUsableAsLadder(SacPlayer player, double x, double y, double z, WrappedBlockState blockData) {
