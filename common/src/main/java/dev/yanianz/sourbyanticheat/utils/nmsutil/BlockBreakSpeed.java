...
import dev.yanianz.sourbyanticheat.utils.data.tags.SyncedTag;
import dev.yanianz.sourbyanticheat.utils.data.tags.SyncedTags;
import dev.yanianz.sourbyanticheat.utils.enums.FluidTag;
import dev.yanianz.sourbyanticheat.utils.inventory.EnchantmentHelper;
...
public class BlockBreakSpeed {
    ...
    public static double getBlockDamage(SacPlayer player, WrappedBlockState block) {
        ItemStack tool = player.inventory.getHeldItem();
        return getBlockDamage(player, tool, block.getType());
    }

    public static double getBlockDamage(SacPlayer player, ItemStack tool, StateType block) {
        // GET destroy speed
    ...
    private static float getSpeedMultiplierFromToolData(SacPlayer player, ItemStack tool, ToolSpeedData data) {
    ...
    private static ToolSpeedData getModernToolSpeedData(SacPlayer player, ItemStack tool, StateType block) {
    ...
    private static ToolSpeedData getLegacyToolSpeedData(SacPlayer player, ItemStack tool, StateType block) {
        ItemType toolType = tool.getType();
        float speedMultiplier = 1.0f;
        boolean isCorrectToolForDrop = false;
