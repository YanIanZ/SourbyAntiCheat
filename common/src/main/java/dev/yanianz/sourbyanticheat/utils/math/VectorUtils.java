...
public class VectorUtils {
    ...
    public static @NotNull Vector3dm cutBoxToVector(double x, double y, double z, @NotNull SimpleCollisionBox box) {
        return new Vector3dm(SacMath.clamp(x, box.minX, box.maxX),
                SacMath.clamp(y, box.minY, box.maxY),
                SacMath.clamp(z, box.minZ, box.maxZ));
    }
    ...
    public static @NotNull Vector3d clampVector(@NotNull Vector3d toClamp) {
        double x = SacMath.clamp(toClamp.getX(), -3.0E7D, 3.0E7D);
        double y = SacMath.clamp(toClamp.getY(), -2.0E7D, 2.0E7D);
        double z = SacMath.clamp(toClamp.getZ(), -3.0E7D, 3.0E7D);
