...
import dev.yanianz.sourbyanticheat.utils.collisions.HitboxData;
...
import dev.yanianz.sourbyanticheat.utils.data.HitData;
...
public class WorldRayTrace {
    public static HitData getNearestBlockHitResult(SacPlayer player, StateType heldItem, boolean sourcesHaveHitbox, boolean fluidPlacement, boolean itemUsePlacement) {
    ...
    public static HitData traverseBlocks(SacPlayer player, Vector3d start, Vector3d end, BiFunction<WrappedBlockState, Vector3i, HitData> predicate) {
        // I guess go back by the collision epsilon?
        double endX = SacMath.lerp(-1.0E-7D, end.x, start.x);
        double endY = SacMath.lerp(-1.0E-7D, end.y, start.y);
        double endZ = SacMath.lerp(-1.0E-7D, end.z, start.z);
        double startX = SacMath.lerp(-1.0E-7D, start.x, end.x);
        double startY = SacMath.lerp(-1.0E-7D, start.y, end.y);
        double startZ = SacMath.lerp(-1.0E-7D, start.z, end.z);
        int floorStartX = SacMath.floor(startX);
        int floorStartY = SacMath.floor(startY);
        int floorStartZ = SacMath.floor(startZ);
        ...
        double d12 = posXInverse * (xSign > 0 ? 1.0D - SacMath.frac(startX) : SacMath.frac(startX));
        double d13 = posYInverse * (ySign > 0 ? 1.0D - SacMath.frac(startY) : SacMath.frac(startY));
        double d14 = posZInverse * (zSign > 0 ? 1.0D - SacMath.frac(startZ) : SacMath.frac(startZ));
