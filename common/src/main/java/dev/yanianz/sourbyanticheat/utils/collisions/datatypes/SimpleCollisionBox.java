...
public class SimpleCollisionBox implements CollisionBox {
    ...
    public Vector3d getCenter() {
        return new Vector3d(SacMath.lerp(0.5, this.minX, this.maxX), SacMath.lerp(0.5, this.minY, this.maxY), SacMath.lerp(0.5, this.minZ, this.maxZ));
    }
    ...
    public static Vector3i containing(double x, double y, double z) {
        return new Vector3i(SacMath.floor(x), SacMath.floor(y), SacMath.floor(z));
    }
    ...
    public static Iterable<Vector3i> betweenCornersInDirection(SimpleCollisionBox boundingBox, Vector3d directionVector) {
        ...
        int minX = SacMath.floor(min.x);
        int minY = SacMath.floor(min.y);
        int minZ = SacMath.floor(min.z);
        Vector3d max = boundingBox.max().toVector3d();
        int maxX = SacMath.floor(max.x);
        int maxY = SacMath.floor(max.y);
        int maxZ = SacMath.floor(max.z);
