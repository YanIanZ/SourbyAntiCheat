...
public class ClientBrand extends Check implements PacketCheck {
    ...
    public ClientBrand(SacPlayer player) {
        super(player);
    }
    ...
    private void handle(String channel, byte[] data) {
        ...
        if (data.length > 64 || data.length == 0) {
            ...
            if (!SacAPI.INSTANCE.getConfigManager().isIgnoredClient(brand)) {
                String message = SacAPI.INSTANCE.getConfigManager().getConfig().getStringElse("client-brand-format", "%prefix% &f%player% joined using %brand%");
                Component component = MessageUtil.replacePlaceholders(player, MessageUtil.miniMessage(message));

                SacAPI.INSTANCE.getAlertManager().sendBrand(component, null);
            }
            ...
            SacAPI.INSTANCE.getDataStoreLifecycle().liveWriteHooks().observeBrandFromCheck(player);
        }
        ...
        if (hasReachHacks && SacAPI.INSTANCE.getConfigManager().isBlockBlacklistedForgeClients()) {
            player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, SacAPI.INSTANCE.getConfigManager().getDisconnectBlacklistedForge())));
        }
