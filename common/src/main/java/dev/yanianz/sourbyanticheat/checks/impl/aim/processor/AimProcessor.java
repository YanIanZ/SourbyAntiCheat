package dev.yanianz.sourbyanticheat.checks.impl.aim.processor;
...
import dev.yanianz.sourbyanticheat.utils.lists.RunningMode;
...
public class AimProcessor extends Check implements RotationCheck {
    ...
    public AimProcessor(SacPlayer playerData) {
        super(playerData);
    }
    ...
    public void process(final RotationUpdate rotationUpdate) {
        ...
        this.divisorX = SacMath.gcd(deltaXRot, lastXRot);
        if (deltaXRot > 0 && deltaXRot < 5 && divisorX > SacMath.MINIMUM_DIVISOR) {
        ...
        this.divisorY = SacMath.gcd(deltaYRot, lastYRot);

        if (deltaYRot > 0 && deltaYRot < 5 && divisorY > SacMath.MINIMUM_DIVISOR) {
