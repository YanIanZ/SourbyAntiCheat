...
public class PredictionEngineNormal extends PredictionEngine {
    ...
    public static void staticVectorEndOfTick(SacPlayer player, Vector3dm vector) {
    ...
    @Override
    public void endOfTick(SacPlayer player, double delta) {
    ...
    public Vector3dm handleOnClimbable(Vector3dm vector, SacPlayer player) {
        if (player.isClimbing) {
            ...
            vector.setX(SacMath.clamp(vector.getX(), -0.15F, 0.15F));
            vector.setZ(SacMath.clamp(vector.getZ(), -0.15F, 0.15F));
