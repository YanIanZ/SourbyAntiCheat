...
import dev.yanianz.sourbyanticheat.predictionengine.PlayerBaseTick;
...
import dev.yanianz.sourbyanticheat.utils.nmsutil.EntityTypeTags;
import dev.yanianz.sourbyanticheat.utils.nmsutil.FluidFallingAdjustedMovement;
import dev.yanianz.sourbyanticheat.utils.nmsutil.GetBoundingBox;
import dev.yanianz.sourbyanticheat.utils.nmsutil.MainSupportingBlockPosFinder;
import dev.yanianz.sourbyanticheat.utils.viaversion.ViaVersionUtil;
import dev.yanianz.sourbyanticheat.utils.team.EntityPredicates;
import dev.yanianz.sourbyanticheat.utils.team.EntityTeam;
...
public class MovementTicker {
    public final SacPlayer player;

    public static void handleEntityCollisions(SacPlayer player) {
    ...
    public void move(Vector3dm inputVel, Vector3dm collide) {
        ...
        if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_18_2)) {
            boolean xAxis = !SacMath.equal(inputVel.getX(), collide.getX());
            boolean zAxis = !SacMath.equal(inputVel.getZ(), collide.getZ());
        ...
        if (collide.lengthSquared() <= 1e-7
            ...
            player.addMovementThisTick(new SacPlayer.Movement(from, to, new Vector3d(inputVel.getX(), inputVel.getY(), inputVel.getZ())));
        }
    ...
    public void livingEntityTravel() {
        ...
        double lavaLevel = 0;
        if (canStandOnLava())
            lavaLevel = player.compensatedWorld.getLavaFluidLevelAt(SacMath.floor(player.lastX), SacMath.floor(player.lastY), SacMath.floor(player.lastZ));
