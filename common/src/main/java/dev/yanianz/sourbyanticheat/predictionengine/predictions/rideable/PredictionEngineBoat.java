...
import dev.yanianz.sourbyanticheat.utils.enums.BoatEntityStatus;
...
public class PredictionEngineBoat extends PredictionEngine {
    public PredictionEngineBoat(SacPlayer player) {
    ...
    private static BoatEntityStatus getStatus(SacPlayer player) {
    ...
    private static @Nullable BoatEntityStatus isUnderwater(@NotNull SacPlayer player) {
        SimpleCollisionBox box = player.boundingBox;
        double maxBoxY = box.maxY + 0.001D;
        int minX = SacMath.floor(box.minX);
        int maxX = SacMath.ceil(box.maxX);
        int minY = SacMath.floor(box.maxY);
        int maxY = SacMath.ceil(maxBoxY);
        int minZ = SacMath.floor(box.minZ);
        int maxZ = SacMath.ceil(box.maxZ);
    ...
    private static boolean checkInWater(SacPlayer grimPlayer) {
        SimpleCollisionBox box = grimPlayer.boundingBox;
        int minX = SacMath.floor(box.minX);
        int maxX = SacMath.ceil(box.maxX);
        int minY = SacMath.floor(box.minY);
        int maxY = SacMath.ceil(box.minY + 0.001D);
        int minZ = SacMath.floor(box.minZ);
        int maxZ = SacMath.ceil(box.maxZ);
    ...
    public static float getGroundFriction(SacPlayer player) {
    ...
    private void floatBoat(SacPlayer player, Vector3dm vector) {
    ...
    public float getWaterLevelAbove(SacPlayer player) {
    ...
    private void controlBoat(SacPlayer player, Vector3dm vector, boolean intermediate) {
        float f = 0.0F;
        ...
        vector.add(new Vector3dm(player.trigHandler.sin(SacMath.radians(-player.yaw)) * f, 0, (double) (player.trigHandler.cos(SacMath.radians(player.yaw)) * f)));
    }
}
