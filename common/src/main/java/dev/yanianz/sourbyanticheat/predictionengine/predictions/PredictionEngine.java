...
import dev.yanianz.sourbyanticheat.predictionengine.movementtick.MovementTickerPlayer;
...
import dev.yanianz.sourbyanticheat.utils.math.Vec2;
...
import dev.yanianz.sourbyanticheat.utils.nmsutil.JumpPower;
import dev.yanianz.sourbyanticheat.utils.nmsutil.Riptide;
...
public class PredictionEngine {
    ...
    public static Vector3dm clampMovementToHardBorder(SacPlayer player, Vector3dm outputVel) {
        // TODO: Reimplement
        return outputVel;
    }

    public static Vector3dm transformInputsToVector(SacPlayer player, Vector3dm theoreticalInput) {
    ...
    public static Vec2 modifyInput(SacPlayer player, Vec2 moveVector) {
    ...
    private static float distanceToUnitSquare(Vec2 input) {
        ...
        return SacMath.sqrt(1.0F + SacMath.square(additional));
    }
    ...
    private static float getItemUseSpeedMultiplier(SacPlayer player) {
    ...
    public void guessBestMovement(float speed, SacPlayer player) {
    ...
    private void doPredictions(SacPlayer player, List<VectorData> possibleVelocities, float speed) {
    ...
    private Pair<Vector3dm, Vector3dm> doSeekingWallCollisions(SacPlayer player, Vector3dm primaryPushMovement, Vector3dm originalClientVel, VectorData clientVelAfterInput) {
    ...
    private void addZeroPointThreeToPossibilities(float speed, SacPlayer player, List<VectorData> possibleVelocities) {
    ...
    public void addFluidPushingToStartingVectors(SacPlayer player, Set<VectorData> data) {
    ...
    public Set<VectorData> fetchPossibleStartTickVectors(SacPlayer player) {
    ...
    private void addNonEffectiveAI(SacPlayer player, Set<VectorData> data) {
    ...
    private void addAttackSlowToPossibilities(SacPlayer player, Set<VectorData> velocities) {
    ...
    public void applyMovementThreshold(SacPlayer player, Set<VectorData> velocities) {
    ...
    public void addExplosionToPossibilities(SacPlayer player, Set<VectorData> existingVelocities) {
    ...
    public int sortVectorData(VectorData a, VectorData b, SacPlayer player) {
        int aScore = 0;
        int bScore = 0;
    ...
    public Vector3dm handleStartingVelocityUncertainty(SacPlayer player, VectorData vector, Vector3dm targetVec) {
    ...
    public void endOfTick(SacPlayer player, double d) {
        player.canSwimHop = canSwimHop(player);
        player.lastWasClimbing = 0;
    }

    private void loopVectors(SacPlayer player, Set<VectorData> possibleVectors, float speed, List<VectorData> returnVectors) {
        // Stop omni-sprint
    ...
    public boolean canSwimHop(SacPlayer player) {
    ...
    public Vector3dm getMovementResultFromInput(SacPlayer player, Vector3dm inputVector, float f, float f2) {
        float f2InRadians = SacMath.radians(f2);
    ...
    public Vector3dm handleOnClimbable(Vector3dm vector, SacPlayer player) {
        return vector;
    }

    public void doJump(SacPlayer player, Vector3dm vector) {
