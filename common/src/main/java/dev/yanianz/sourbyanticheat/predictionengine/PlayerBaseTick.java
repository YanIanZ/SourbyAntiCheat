package dev.yanianz.sourbyanticheat.predictionengine;

import dev.yanianz.sourbyanticheat.player.SacPlayer;
import dev.yanianz.sourbyanticheat.utils.collisions.datatypes.SimpleCollisionBox;
import dev.yanianz.sourbyanticheat.utils.data.attribute.ValuedAttribute;
...
import dev.yanianz.sourbyanticheat.utils.latency.CompensatedEntities;
...
import dev.yanianz.sourbyanticheat.utils.nmsutil.CheckIfChunksLoaded;
import dev.yanianz.sourbyanticheat.utils.nmsutil.Collisions;
import dev.yanianz.sourbyanticheat.utils.nmsutil.FluidTypeFlowing;
...
public final class PlayerBaseTick {
    ...
    public static boolean canEnterPose(SacPlayer player, Pose pose, double x, double y, double z) {
        return Collisions.isEmpty(player, getBoundingBoxForPose(player, pose, x, y, z).expand(-1.0E-7D));
    }

    private static SimpleCollisionBox getBoundingBoxForPose(SacPlayer player, Pose pose, double x, double y, double z) {
    ...
    public static void doBaseTick(SacPlayer player) {
    ...
    private static void updateFluidOnEyes(SacPlayer player) {
    ...
    private static void updateInWaterStateAndDoFluidPushing(SacPlayer player) {
    ...
    public static void updatePowderSnow(SacPlayer player) {
    ...
    // 1.14
    public static void updatePlayerPose(SacPlayer player) {
    ...
    // 1.13 and below
    private static void updatePlayerSize(SacPlayer player) {
        Pose pose;
        if (player.isGliding) {
            pose = Pose.FALL_FLYING;
        } else if (player.isInBed) {
    ...
    private static void updateSwimming(SacPlayer player) {
    ...
    private static void moveTowardsClosestSpace(SacPlayer player, double xPosition, double zPosition) {
    ...
    private static void moveTowardsClosestSpaceLegacy(SacPlayer player, double x, double z) {
        int floorX = SacMath.floor(x);
        int floorZ = SacMath.floor(z);
        int floorY = SacMath.floor(player.lastY + 0.5);
    ...
    // 1.14+
    private static void moveTowardsClosestSpaceModern(SacPlayer player, double xPosition, double zPosition) {
    ...
    public static void updateInWaterStateAndDoWaterCurrentPushing(SacPlayer player) {
        final PacketEntity riding = player.compensatedEntities.self.getRiding();
    ...
    private static boolean updateFluidHeightAndDoFluidPushing(SacPlayer player, FluidTag tag, double multiplier) {
    ...
    private static boolean updateFluidHeightAndDoFluidPushingLegacy(SacPlayer player, FluidTag tag, double multiplier) {
        SimpleCollisionBox aABB = player.boundingBox.copy().expand(0, -0.4, 0).expand(-0.001);

        int floorX = SacMath.floor(aABB.minX);
        int ceilX = SacMath.ceil(aABB.maxX);
        int floorY = SacMath.floor(aABB.minY);
        int ceilY = SacMath.ceil(aABB.maxY);
        int floorZ = SacMath.floor(aABB.minZ);
        int ceilZ = SacMath.ceil(aABB.maxZ);
    ...
    private static boolean updateFluidHeightAndDoFluidPushingModern(SacPlayer player, FluidTag tag, double multiplier) {
    ...
    public static boolean updateFluidInteraction(SacPlayer player) {
    ...
    private static boolean suffocatesAt(SacPlayer player, int x, int z) {
    ...
    private static boolean clearAbove(SacPlayer player, int x, int y, int z) {
