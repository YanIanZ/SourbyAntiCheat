...
import dev.yanianz.sourbyanticheat.checks.impl.vehicle.VehicleC;
import dev.yanianz.sourbyanticheat.checks.type.PositionCheck;
import dev.yanianz.sourbyanticheat.manager.SetbackTeleportUtil;
import dev.yanianz.sourbyanticheat.player.SacPlayer;
import dev.yanianz.sourbyanticheat.predictionengine.movementtick.MovementTickerCamel;
import dev.yanianz.sourbyanticheat.predictionengine.movementtick.MovementTickerHappyGhast;
import dev.yanianz.sourbyanticheat.predictionengine.movementtick.MovementTickerHorse;
import dev.yanianz.sourbyanticheat.predictionengine.movementtick.MovementTickerNautilus;
import dev.yanianz.sourbyanticheat.predictionengine.movementtick.MovementTickerPig;
import dev.yanianz.sourbyanticheat.predictionengine.movementtick.MovementTickerPlayer;
import dev.yanianz.sourbyanticheat.predictionengine.movementtick.MovementTickerStrider;
import dev.yanianz.sourbyanticheat.predictionengine.predictions.PredictionEngineNormal;
import dev.yanianz.sourbyanticheat.predictionengine.predictions.rideable.PredictionEngineBoat;
import dev.yanianz.sourbyanticheat.predictionengine.predictions.rideable.PredictionEngineRideableUtils;
...
import dev.yanianz.sourbyanticheat.utils.data.SetBackData;
...
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntityTrackXRot;
...
import dev.yanianz.sourbyanticheat.utils.nmsutil.BoundingBoxSize;
...
public class MovementCheckRunner extends Check implements PositionCheck {
    ...
    public MovementCheckRunner(SacPlayer player) {
        super(player);
    }
    ...
    private void check(PositionUpdate update) {
        ...
        if (!player.isFlying && player.isSneaking && Collisions.isAboveGround(player)) {
            ...
            double posX = Math.max(0.05, SacMath.clamp(player.actualMovement.getX(), -16, 16) + 0.05);
            double posZ = Math.max(0.05, SacMath.clamp(player.actualMovement.getZ(), -16, 16) + 0.05);
            double negX = Math.min(-0.05, SacMath.clamp(player.actualMovement.getX(), -16, 16) - 0.05);
            double negZ = Math.min(-0.05, SacMath.clamp(player.actualMovement.getZ(), -16, 16) - 0.05);
