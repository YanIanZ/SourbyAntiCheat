package dev.yanianz.sourbyanticheat.player;
...
import dev.yanianz.sourbyanticheat.manager.player.handlers.DefaultResyncHandler;
import dev.yanianz.sourbyanticheat.manager.player.handlers.NoOpResyncHandler;
import dev.yanianz.sourbyanticheat.platform.api.player.PlatformPlayer;
import dev.yanianz.sourbyanticheat.predictionengine.EntityFluidInteraction;
import dev.yanianz.sourbyanticheat.predictionengine.MovementCheckRunner;
import dev.yanianz.sourbyanticheat.predictionengine.PointThreeEstimator;
import dev.yanianz.sourbyanticheat.predictionengine.UncertaintyHandler;
import dev.yanianz.sourbyanticheat.manager.AttackCooldownHandler;
...
import dev.yanianz.sourbyanticheat.utils.change.PlayerBlockHistory;
import dev.yanianz.sourbyanticheat.utils.collisions.datatypes.SimpleCollisionBox;
import dev.yanianz.sourbyanticheat.utils.data.*;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntity;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntityHappyGhast;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntityNautilus;
import dev.yanianz.sourbyanticheat.utils.data.packetentity.PacketEntitySelf;
...
public class SacPlayer implements GrimUser {
    ...
    @Getter @Setter private ResyncHandler resyncHandler = SacAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("disable-default-resync-handler", false) ? NoOpResyncHandler.INSTANCE : new DefaultResyncHandler(this);
    ...
    public SacPlayer(@NotNull User user) {
        this.user = user;
    ...
    public void onPacketCancel() {
        if (spamThreshold != -1 && cancelledPackets.incrementAndGet() > spamThreshold) {
            ...
            disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, SacAPI.INSTANCE.getConfigManager().getDisconnectClosed())));
    ...
    public void timedOut() {
        disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, SacAPI.INSTANCE.getConfigManager().getDisconnectTimeout())));
    }
    ...
    public void disconnect(Component reason) {
        ...
        if (platformPlayer != null) {
            SacAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, SacAPI.INSTANCE.getGrimPlugin(),
    ...
    public void pollData() {
        ...
        if (!SacAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            SacAPI.INSTANCE.getPlayerDataManager().remove(user);
        }
        ...
        if (uuid != null && this.platformPlayer == null) {
            this.platformPlayer = SacAPI.INSTANCE.getPlatformPlayerFactory().getFromUUID(uuid);
            updatePermissions();
        }
        ...
        if (uuid != null) {
            SacAPI.INSTANCE.getDataStoreLifecycle().sessionTracker()
    ...
    public void updatePermissions() {
        ...
        try {
            SacAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, SacAPI.INSTANCE.getGrimPlugin(), () -> {
                this.noModifyPacketPermission = platformPlayer.hasPermission("sac.nomodifypacket");
                this.noSetbackPermission = platformPlayer.hasPermission("sac.nosetback");
    ...
    public int getTransactionPing() {
        return SacMath.floor(transactionPing / 1e6);
    }
    ...
    public final void reload(ConfigManager config) {
        ...
        maxTransactionTime = SacMath.clamp(config.getIntElse("max-transaction-time", 60), 1, 180);
    ...
    public void resyncPositions(SimpleCollisionBox box) {
        this.resyncHandler.resync(SacMath.floor(box.minX), SacMath.floor(box.minY), SacMath.floor(box.minZ),
                SacMath.ceil(box.maxX), SacMath.ceil(box.maxY), SacMath.ceil(box.maxZ));
    }

    public void addMovementThisTick(SacPlayer.Movement movement) {
        if (this.movementThisTick.size() >= 100) {
            SacPlayer.Movement movement1 = this.movementThisTick.removeFirst();
            SacPlayer.Movement movement2 = this.movementThisTick.removeFirst();
            SacPlayer.Movement movement3 = new SacPlayer.Movement(movement1.from(), movement2.to());
