...
public final class LiveWriteHooksImpl implements LiveWriteHooks {
    ...
    public void onJoinFromUserLogin(@NotNull PlatformPlayer player, @NotNull User user, long now) {
        SacPlayer gp = SacAPI.INSTANCE.getPlayerDataManager().getPlayer(user);
        onJoin(player.getUniqueId(), player.getName(), now, LiveWriteHooks.clientMetaFor(user, gp));
    }

    @Override
    public void onQuitFromUserDisconnect(@NotNull User user, @Nullable SacPlayer grimPlayer, long now) {
    ...
    @Override
    public void observeBrandFromCheck(@NotNull SacPlayer grimPlayer) {
