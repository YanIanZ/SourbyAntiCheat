...
import dev.yanianz.sourbyanticheat.checks.impl.badpackets.BadPacketsN;
import dev.yanianz.sourbyanticheat.checks.type.PostPredictionCheck;
import dev.yanianz.sourbyanticheat.platform.api.entity.SacEntity;
import dev.yanianz.sourbyanticheat.player.SacPlayer;
import dev.yanianz.sourbyanticheat.predictionengine.predictions.PredictionEngine;
import dev.yanianz.sourbyanticheat.predictionengine.predictions.PredictionEngineElytra;
...
import dev.yanianz.sourbyanticheat.utils.data.TeleportAcceptData;
import dev.yanianz.sourbyanticheat.utils.data.TeleportData;
import dev.yanianz.sourbyanticheat.utils.data.VectorData;
import dev.yanianz.sourbyanticheat.utils.data.VelocityData;
...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
    ...
    private static final GrimTeleportEvent.Channel TELEPORT_CHANNEL = SacAPI.INSTANCE.getEventBus().get(GrimTeleportEvent.class);
    private static final GrimPlayerSetbackEvent.Channel PLAYER_SETBACK_CHANNEL = SacAPI.INSTANCE.getEventBus().get(GrimPlayerSetbackEvent.class);
    ...
    public SetbackTeleportUtil(SacPlayer player) {
        super(player);
    }
        ...
        try {
            ...
            if (player.inVehicle()) {
                ...
                if (player.compensatedEntities.serverPlayerVehicle != null) {
                    ...
                    SacAPI.INSTANCE.getScheduler().getEntityScheduler().execute(player.platformPlayer, SacAPI.INSTANCE.getGrimPlugin(), () -> {
                        if (player.platformPlayer != null) {
                            SacEntity vehicle = player.platformPlayer.getVehicle();
                            if (vehicle != null) {
                                vehicle.eject();
                            }
                        }
    ...
    public boolean insideUnloadedChunk() {
        Column column = player.compensatedWorld.getChunk(SacMath.floor(player.x) >> 4, SacMath.floor(player.z) >> 4);
