...
import dev.yanianz.sourbyanticheat.manager.init.ReloadableInitable;
...
import dev.yanianz.sourbyanticheat.utils.data.webhook.discord.Embed;
import dev.yanianz.sourbyanticheat.utils.data.webhook.discord.EmbedField;
import dev.yanianz.sourbyanticheat.utils.data.webhook.discord.EmbedFooter;
...
public class DiscordManager implements StartableInitable, ReloadableInitable {
    ...
    private static String validatedConfigURL(String configPath, String defaultURL) {
        String url = SacAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-image-url", defaultURL);
    ...
    public void reload() {
        try {
            ...
            ConfigManager config = SacAPI.INSTANCE.getConfigManager().getConfig();
    ...
    public void sendAlert(@NotNull SacPlayer player, String verbose, String checkName, int violations) {
        if (isDisabled()) {
            return;
        }

        // Per-alert overlay — avoids polluting the global static map
        Map<String, String> statics = new HashMap<>(SacAPI.INSTANCE.getExternalAPI().getStaticReplacements());
        statics.put("%check%", checkName);
        statics.put("%violations%", Integer.toString(violations));

        Map<String, Function<GrimUser, String>> dynamics = SacAPI.INSTANCE.getExternalAPI().getVariableReplacements();
    ...
    public CompletableFuture<Boolean> sendWebhookMessage(WebhookMessage message) {
        ...
        if (!taskStarted.getAndSet(true)) {
            ...
            SacAPI.INSTANCE.getScheduler().getAsyncScheduler().runAtFixedRate(SacAPI.INSTANCE.getGrimPlugin(), DiscordManager::tick, 0, 1);
        }
