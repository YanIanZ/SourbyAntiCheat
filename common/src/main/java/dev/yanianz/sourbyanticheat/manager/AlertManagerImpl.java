...
public final class AlertManagerImpl implements AlertManager, ConfigReloadable, StartableInitable {
    ...
    @Override
    public void start() {
        platformServer = SacAPI.INSTANCE.getPlatformServer();
     ...
     * @throws IllegalArgumentException if the user is not a SacPlayer.
     * @throws NullPointerException if the SacPlayer's platformPlayer is null.
     */
    private @NotNull PlatformPlayer requirePlatformPlayerFromUser(@NotNull GrimUser user) {
        ...
        if (!(user instanceof SacPlayer grimPlayer)) {
            // Throw a specific exception if the type is wrong
            throw new IllegalArgumentException("AlertManager action called with non-SacPlayer user: " + user.getName());
        }
    ...
    public boolean hasBrandsEnabled(@NotNull GrimUser player) {
        SacPlayer grimPlayer = (SacPlayer) player;
    ...
    public boolean hasAlertsEnabled(Player player) {
        ...
        return hasAlertsEnabled(SacAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleAlerts(Player player) {
        if (player == null) return;
        toggleAlerts(SacAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public boolean hasVerboseEnabled(Player player) {
        if (player == null) return false;
        return hasVerboseEnabled(SacAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleVerbose(Player player) {
        if (player == null) return;
        toggleVerbose(SacAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
