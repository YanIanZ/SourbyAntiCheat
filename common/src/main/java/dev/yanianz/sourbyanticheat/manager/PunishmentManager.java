...
public class PunishmentManager implements ConfigReloadable {
    ...
    public boolean handleAlert(SacPlayer player, String verbose, Check check) {
        ...
        for (PunishGroup group : groups) {
            if (group.checks.contains(check)) {
                ...
                for (ParsedCommand command : group.commands) {
                    ...
                    if (SacAPI.INSTANCE.getAlertManager().hasVerboseListeners() && command.command.equals("[alert]")) {
                        sentDebug = true;
                        Component component = MessageUtil.miniMessage(cmd);
                        verboseListeners = SacAPI.INSTANCE.getAlertManager().sendVerbose(component, null);
                    }
                    ...
                    if (violationCount >= command.threshold) {
                        ...
                        if (inInterval) {
                            ...
                            switch (command.command) {
                                case "[webhook]" -> SacAPI.INSTANCE.getDiscordManager().sendAlert(player, verbose, check.getDisplayName(), vl);
                                case "[log]" -> {
                                ...
                                default -> SacAPI.INSTANCE.getScheduler().getGlobalRegionScheduler().run(SacAPI.INSTANCE.getGrimPlugin(), () ->
                                        SacAPI.INSTANCE.getPlatformServer().dispatchCommand(
                                                SacAPI.INSTANCE.getPlatformServer().getConsoleSender(),
                                                cmd
                                        )
                                );
                            }
