package dev.yanianz.sourbyanticheat.manager.player.handlers;
...
import dev.yanianz.sourbyanticheat.platform.api.world.PlatformChunk;
...
public record DefaultResyncHandler(SacPlayer player) implements ResyncHandler {

    private static void resyncPositions(SacPlayer player, int minBlockX, int mY, int minBlockZ, int maxBlockX, int mxY, int maxBlockZ) {
        ...
        SacAPI.INSTANCE.getScheduler().getRegionScheduler().execute(SacAPI.INSTANCE.getGrimPlugin(), world,
    ...
    private static void resyncPosition(SacPlayer player, int x, int y, int z, int sequence) {
        if (player.platformPlayer == null) return;
        ...
        SacAPI.INSTANCE.getScheduler().getRegionScheduler().execute(SacAPI.INSTANCE.getGrimPlugin(), world, chunkX, chunkZ, () -> {
