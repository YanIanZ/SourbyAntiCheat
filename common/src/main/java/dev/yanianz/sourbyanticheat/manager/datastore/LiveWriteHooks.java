package dev.yanianz.sourbyanticheat.manager.datastore;
...
public interface LiveWriteHooks {
    ...
    /** Convenience for {@code PacketPlayerJoinQuit.onUserLogin}. Builds meta from the User + SacPlayer internally. */
    ...
    void onQuitFromUserDisconnect(@NotNull User user, @Nullable SacPlayer grimPlayer, long now);

    /** Convenience for {@code ClientBrand.onPacketReceive}. Pulls UUID + builds meta from the SacPlayer internally. */
    void observeBrandFromCheck(@NotNull SacPlayer grimPlayer);
     ...
     * for {@code user}. Reads from the passed-in {@link SacPlayer} (when
    ...
    static @NotNull SessionTracker.ClientMeta clientMetaFor(
            @NotNull User user, @Nullable SacPlayer gp) {
        String grimVersion = SacAPI.INSTANCE.getExternalAPI().getSacVersion();
        int clientVersionPvn = user.getClientVersion().getProtocolVersion();
        String serverVersion = SacAPI.INSTANCE.getPlatformServer().getPlatformImplementationString();
        String brand = null;
        if (gp != null) {
    ...
    LiveWriteHooks NOOP = new LiveWriteHooks() {
        ...
        @Override public void onQuitFromUserDisconnect(@NotNull User u, @Nullable SacPlayer g, long t) {}
        @Override public void observeBrandFromCheck(@NotNull SacPlayer g) {}
        @Override public void recordFlagFromCheck(@NotNull SacPlayer p, @NotNull AbstractCheck c, double v, @Nullable String vb) {}
    };
}
